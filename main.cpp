...
int main(int argc, char* argv[])
{
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        if (GuiButton(Rectangle { 25, 60, 125, 30 }, GuiIconText(RICON_OK_TICK, "Insert")))
        {
            // Implement the code to insert valueBoxValue into your binary tree here! 
            m_binaryTree.Insert(valueBoxValue);
        }

        if (GuiButton(Rectangle{ 25, 95, 125, 30 }, GuiIconText(RICON_CROSS, "Remove")))
        {
        ...
        if (GuiButton(Rectangle{ 25, 130, 125, 30 }, GuiIconText(RICON_EYE_ON, "Find")))
        {
            // Find and highlight value
